...
| :--- | :--- | :--- | :--- |
| **CCAPI-27.2** | 2025-10-27 | Chris Moore | **📊 BENCHMARK DRIFT MONITORING SYSTEM (v2.9.0) - DEPLOYED TO PRODUCTION:** Automated drift tracking system preventing RVI model degradation by monitoring divergence between static tier benchmarks and live market prices. **Core Algorithm:** Drift calculation `(live_price - benchmark) / benchmark * 100`, persistence-based alert thresholds (WARNING: >10% drift for 4+ consecutive weeks, CRITICAL: >20% drift for 2+ consecutive weeks), weekly snapshot tracking with 6-month rolling retention (180 days configurable). **Architecture:** `BenchmarkDriftMonitor` class (608 lines) with 8 core methods: `calculate_drift()` (single region calculation), `track_drift()` (batch weekly processing), `_save_drift_snapshot()` (JSON persistence with TTL cleanup), `get_drift_history()` (query historical snapshots), `_check_drift_alerts()` (consecutive week detection), `get_tier_drift_summary()` (tier-level aggregation with 30-day smoothing), `generate_recalibration_recommendations()` (propose new benchmarks), plus 6 helper methods for data extraction. **Data Model:** DriftSnapshot schema: `{timestamp, region_name, tier, benchmark_price, live_price, drift_pct, data_source, confidence, alert_level}`. DriftAlert schema: `{region, tier, current_drift_pct, consecutive_weeks, alert_level, first_detected, avg_drift_pct, recommendation}`. Storage: JSON files `./data/benchmark_drift/{region}_drift_history.json`, automatic cleanup beyond retention window. **Weekly Monitoring Integration:** Added 3-section integration to `run_weekly_java_monitor.py`: (1) Post-monitoring drift tracking (after line 126) - imports `BenchmarkDriftMonitor`, initializes with 6-month retention, calls `track_drift(regions_analyzed)`, logs drift alerts, adds `drift_summary` to results dict, non-blocking error handling. (2) Results output section (after line 152) - displays drift stats: regions tracked, average drift, regions >10%/20%, active alerts, top 5 drift alerts with region/drift%/consecutive weeks. (3) Output files section (after line 218) - added drift history file paths. **Admin Recalibration Tool:** `tools/recalibrate_benchmarks.py` (512 lines, executable) with `BenchmarkRecalibrator` class supporting 4 workflows: `review` (display current drift status for all tiers with 🟢 HEALTHY, 🟡 WARNING, 🔴 CRITICAL indicators), `propose` (generate recommendations using 30-day smoothed drift, save to JSON proposal file), `apply` (update `market_config.py` benchmarks with timestamped backup to `data/benchmark_backups/`), `rollback` (restore previous configuration from backup). Workflow: review drift → validate data quality → propose new benchmarks → generate diff report → interactive confirmation → apply updates → log to recalibration history. **Safety Features:** Timestamped backups before any changes, recalibration history log (JSON), user confirmation prompts, git-versionable updates. **Performance Metrics:** <5 min overhead per 29-region run (zero additional API calls - uses existing live prices from weekly monitoring), ~10MB storage per region per 6 months (~290MB total for 29 regions), append-only operations preserving full history. **Files Changed:** `src/core/benchmark_drift_monitor.py` (NEW - 608 lines), `run_weekly_java_monitor.py` (MODIFIED - 3 integration sections), `tools/recalibrate_benchmarks.py` (NEW - 512 lines, executable). **Deployment:** Git commit 2ec16b0, pushed to GitHub main branch October 27, 2025. **Status:** ✅ **DEPLOYED TO PRODUCTION - Awaiting validation testing and first production weekly cycle** |
